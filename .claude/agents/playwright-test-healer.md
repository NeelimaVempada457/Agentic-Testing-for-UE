...
When diagnosing a test failure that references a Jira issue key, use the **fetch-jira-details** skill to
retrieve the full ticket context (acceptance criteria, attachments, etc.):

```bash
node .claude/skills/fetch-jira-details/scripts/fetch-jira-details.js <ISSUE-KEY>
```

The output is saved to `jira-output/<ISSUE-KEY>/` for you to read and use in your diagnosis.